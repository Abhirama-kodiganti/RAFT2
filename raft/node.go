...
import (
	...
	"github.com/Abhirama-kodiganti/raft3d/api/models"
	...
	if config.Bootstrap {
		...
		for _, peer := range config.Peers {
			if peer != config.RaftAddr {
				var port int
				_, err := fmt.Sscanf(peer[len(peer)-4:], "%d", &port)
				if err != nil {
					continue
				}
				
				nodeNumber := port - 7000 + 1
				
				configuration.Servers = append(configuration.Servers, raft.Server{
					ID:      raft.ServerID(fmt.Sprintf("node%d", nodeNumber)),
